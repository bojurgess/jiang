...
bundler:
	...
	wasm-opt -Oz --enable-bulk-memory -o pkg/bundler/jiang_bg.wasm pkg/bundler/jiang_bg.wasm

web:
	wasm-pack build --target web --scope bojurgess --out-dir pkg/web
	wasm-opt -Oz --enable-bulk-memory -o pkg/web/jiang_bg.wasm pkg/web/jiang_bg.wasm

node:
	wasm-pack build --target nodejs --scope bojurgess --out-dir pkg/node
	wasm-opt -Oz --enable-bulk-memory -o pkg/node/jiang_bg.wasm pkg/node/jiang_bg.wasm
